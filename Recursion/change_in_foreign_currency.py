...
# it's actually impossible to receive change for a given amount of money. For example, Canada has given up the 1-cent penny. 
...
def canGetExactChange(targetMoney, denominations):
  ...
  def helper(idx):
    ...
    if curr == targetMoney:
      res = True
      return
    if curr > targetMoney or idx > len(denominations)-1:
      return
    curr = curr + denominations[idx]
    helper(idx)
    curr = curr - denominations[idx]
    helper(idx+1)
